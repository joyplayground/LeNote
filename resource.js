...
function getRenderPages() {
  ...
  return items
    .filter(item => {
      return fs.existsSync(join(item.path, "index.tsx"));
    })
    .map(item => {
    ...
    .map(item => {
      ...
      const js = join(render_dir, item, "index.tsx");
...
function getAssets() {
  ...
  return items
    .filter(item => /\.(js|css|png|jpg|gif|webp|ico|eot|svg|ttf|woff2?)$/.test(item.path))
    .map(item => {
